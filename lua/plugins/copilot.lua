return {
	{
		...
		config = function()
			...
			local function system_stats()
				local handle = io.popen("free -h | grep Mem | awk '{print $3}'")
				local ram_usage = handle:read("*a"):gsub("\n", "")
				handle:close()
				local handle_disk = io.popen("df -h | grep /dev/sda1 | awk '{print $5}'")
				local disk_usage = handle_disk:read("*a"):gsub("\n", "")
				handle_disk:close()
				return string.format("💾 %s RAM   %s Disk", ram_usage, disk_usage)
			end

			local function gradient_bg()
				-- Create a psychedelic gradient effect
				return {
					fg = colors.fg,
					bg = string.format(
						"#%02X%02X%02X",
			...
			-- Apply crazy themes with gradient and visual effects
			...
			require("lualine").setup({
				...
				lualine_z = {
					...
					},
					{
						...
						color = { fg = colors.neon_pink },
						separator = { left = "", right = "" },
					},
					{
						system_stats,
						separator = { left = "", right = "" },
						color = { fg = colors.bg_dark, bg = colors.neon_blue, gui = "bold" },
					},
					{
						lsp_status,
						separator = { left = "", right = "" },
